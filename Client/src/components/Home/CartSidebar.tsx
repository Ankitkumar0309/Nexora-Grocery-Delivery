            ...
            {items.length === 0 ? (
              ...
              items.map((item)=>(
                <div key={item.product._id} className="flex gap-3 bg-app-cream/60 rounded-xl p-3">
                  <img src={item.product.image} alt={item.product.name} className="size-16 rounded-lg
                  object-cover shrink-0"/>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs text-app-text-light">
          ...
          {items.length > 0 && (
            <div className="p-5 border-t border-app-border space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{deliveryFree === 0 ? <span 
                className="text-app-success">Free</span> : `${currency}${deliveryFree.toFixed(2)}`}</span>
              </div>

              {deliveryFree > 0 && <p className="text-xs text-app-text-light text-center">
              Free delivery on orders over {currency}20!</p>}

              <div className="flex justify-between text-base font-base font-semibold border-t border-app-border pt-3">
                <span>Total</span>
