...
const Navbar = () => {
    const user: any = {name: "john Deo", email: "john@example.com", isAdmin: true}
            ...
            <div className="w-full flex items-center justify-end gap-4 lg:gap-10">
                ...
                <form onSubmit={handleSearch} className="hidden sm:flex flex-1 max-w-sm text-xs sm:text-sm">
                    <div className="relative w-full">
                        ...
                        -translate-y-1/2 size-4 size-4 text-zinc-500"/>
                        <input type="text" placeholder="Search for groceries..." value={searchQuery} 
                        onChange={(e)=>setSearchQuery(e.target.value)} className="w-full pl-8 p-2 bg-orange-50
                        rounded-full ring ring-app-orange/15 focus:ring-app-orange/30"/>
                    </div>
                </form>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="relative p-2 rounded-xl" onClick={()=>setIsCartOpen(true)}>
                        ...
                        {cartCount > 0 && <span className="absolute -top-1 -right-1 size-4 bg-app-orange
                        text-white text-[10px] rounded-full flex-center">{cartCount}</span>}
                    </button>
                    {/* User */}
                    <div className="relative">
                        ...
                        {userMenuOpen && (
                            <>
                               <div className="fixed insert-0 z-40" onClick={()=>
                                setUserMenuOpen(false)} />
                                ...
                                border-app-border py-2 z-50 animate-fade-in">
                                    ...
                                    {user?.isAdmin && (
                                        <Link to='/admin/products' className="dropdown-link md:hidden"><ShieldIcon className="text-app-orange" size={16} /> 
