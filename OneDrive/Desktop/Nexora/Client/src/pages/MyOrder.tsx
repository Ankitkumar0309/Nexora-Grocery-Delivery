    ...
    <div className="min-h-screen bg-app-cream mb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {tabs.map((tab)=>(
            <button key={tab} onClick={()=>setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium rounded-xl
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {orders.map((order)=>(
              <Link key={order._id} to={`/orders/${order._id}`} className="block max-w-4xl 
              bg-white rounded-2xl p-5 hover:shadow transition-all">
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-app-text-light">{new Date(order.createdAt).toLocaleDateString("en-US", {month: "short",
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-4 py-1 text-xs font-xs font-medium rounded-full
                ...
                <div className="flex items-center gap-2 mb-3">
                  {order.items.slice(0,4).map((item, i)=>(
                    ...
                    className="size-12 sm:size-16 rounded-lg object-cover border boder-app-border"/>
                  ))}
                  {order.items.length > 4 && <div className="size-12 sm:size-16 rounded-lg bg-app-cream 
